...
class InMemoryMedicationNameIndexTest {
    ...
    private val testMedications = listOf(
        "Amoxicillin", "Clavamox", "Cephalexin", "Gabapentin", "Meloxicam",
        "Prednisone", "Metronidazole", "Carprofen", "Rimadyl", "Tramadol"
    )

    private fun createIndex(medications: List<String> = testMedications) =
        InMemoryMedicationNameIndex(medications)

    @Test
    fun search_exactMatch_returnsAsFirstResult() = runTest {
    ...
    @Test
    fun search_prefixMatch_returnsMatchingNames() = runTest {
        val index = createIndex()
    ...
    @Test
    fun search_substringMatch_returnsMatchingNames() = runTest {
        val index = createIndex()
    ...
    @Test
    fun search_caseInsensitive_matchesRegardlessOfCase() = runTest {
        val index = createIndex()
    ...
    @Test
    fun search_limitsResultsToSpecifiedCount() = runTest {
        val index = createIndex()
    ...
    @Test
    fun search_blankQuery_returnsAllLimited() = runTest {
        val index = createIndex()
    ...
    @Test
    fun search_emptyQuery_returnsAllLimited() = runTest {
        val index = createIndex()
        val results = index.search("")
        assertEquals(10, results.size) // default limit
    }
    ...
    @Test
    fun search_ordering_exactBeforePrefixBeforeSubstring() = runTest {
    ...
    @Test
    fun getAll_returnsAllSortedAlphabetically() = runTest {
