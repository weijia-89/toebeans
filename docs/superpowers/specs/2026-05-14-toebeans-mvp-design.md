...
No user-acquisition or revenue criteria in scope for milestone 1.
...
toebeans/
  ...
  shared/                        # KMP shared module
    ...
    src/iosMain/kotlin/          # placeholder for milestone 5
  androidApp/
...
| internal-test | Play Console internal test track | future, milestone 6 |
| production | Play Store closed/open testing → production | future |

**Secrets:** none in v1 (no cloud).
**CI steps (milestone 1):**
1. ktlint
2. detekt
3. unit tests (commonTest + android unit tests + Robolectric slices in CI)
4. **fitness functions** (see §9)
5. Trainer PR review comment gate (required on code PRs; docs/research-only markdown PRs are exempt)
6. Build debug APK
7. Optional operator-run smoke/soak checks outside merge gate (see beta docs)

**SLSA target:** L1 at v1 (provenance generated by GitHub Actions); L2 is the goal at milestone 6.
...
Enforced in CI on every PR (plus trainer comment gate on non-doc PRs):

1. **No-network gate.** A unit test fails the build if any class in `commonMain/` or `androidMain/` (milestone 1) imports `okhttp3`, `ktor`, `java.net.URL`, `HttpURLConnection`, `URLSession`, or similar. Implementation: `tests/test_no_network.sh` greps the dependency graph and source tree.
...
| **T: Tampering** | local DB; backup file | v1 backup export/import is plain JSON (ADR-0016). File integrity depends on app-private DB + SAF destination choice; encrypted passphrase envelope is deferred to ADR-0016 v2 trigger. Android Auto Backup encryption remains platform-provided (per-user device-bound key). | `[verified]` |
| **R: Repudiation** | DoseEvent log | n/a (single-user, no claims to repudiate) | `[verified]` |
| **I: Info disclosure** | backup file leaked; device shared | v1 backup is not passphrase-encrypted; posture relies on low-sensitivity schema + user-controlled SAF location. Settings should still warn before enabling Android Auto Backup that data lands in the user's Google account. | `[verified]` |
...
7. **Q:** What feature will users ask for that we'll defer to milestone 2? **A:** Caregiver sharing; multi-device sync.
