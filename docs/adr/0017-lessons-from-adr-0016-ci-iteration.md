...
- **`scripts/test_no_duplicate_private_test_class_names.sh`.** The fitness function sketched in Lesson 2, plus the `scripts/test_pre_commit_hook.sh`-style negative test cases that verify it fires on the `4ee14a2` shape and passes on the `fa61a40` shape. The CI workflow `.github/workflows/ci.yml` would also need a job entry to invoke the new script; that integration is part of the same follow-on session.
    - → Landed in commit `01fe604`. The fitness function scans every Kotlin test source root that exists in the repo, extracts each file's `package` declaration and every `^private class <Name>` line, and reports any `(package, classname)` pair that appears in more than one file. Six fixtures cover positive (synthetic duplicate triggers a failure) and negative (single declaration, distinct classnames, same name across different packages, `private open class`, nested `private class`) cases, all run as a CI self-test step. First green CI run for this script is blocked by an in-tree finding: `ExportBackupViewModelTest.kt` and `ImportBackupViewModelTest.kt` (both in `androidApp/src/test/kotlin/app/toebeans/android/ui/settings/`) each declare `InMemoryPetRepo`, `InMemoryMedRepo`, `InMemoryScheduleRepo`, and `InMemoryDoseEventRepo` in the same package. These compile green under the current Android-test toolchain (CI run 26039809143 was clean), so the K2 visibility behavior that fired in `shared/src/commonTest/.../backup/` evidently does not fire in `androidApp/src/test/` today. The fitness function still flags them as latent risk. The rename of those four fakes is the next follow-on task; until it ships, the new CI step lands red on this branch and the merge is blocked behind that rename.
...
- **`.github/workflows/ci.yml` rework for independent lint and compile-tests gates.** The local-gauntlet ordering landed via `CONTRIBUTING.md` mitigates the cost per contributor but does not close the loop. The CI workflow itself still runs the Lint stage before the Gradle compile step and short-circuits the latter on the former's failure, so any contributor without local JDK 17 inherits the same inference problem the new `CONTRIBUTING.md` warns against. The remediation is either a fan-out workflow (two parallel jobs both consuming the checkout artifact) or reordering inside the existing single-job gauntlet so a Lint failure does not skip downstream gates. The trade-off against the existing GitHub Actions parallelism budget is the reason this lesson was originally scoped to the local gauntlet only, per Lesson 1 above. Revisit condition #3 in this ADR also covers the upstream case where GitHub Actions adds a native fan-out option that changes the trade-off.
