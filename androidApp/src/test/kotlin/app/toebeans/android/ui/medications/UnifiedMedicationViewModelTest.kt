...
class UnifiedMedicationViewModelTest {
    private fun createViewModel(
        ...
        return UnifiedMedicationViewModel(
    ...
    @Test
    fun `save valid state creates medication and schedule`() = runTest {
    ...
    @Test
    fun `save blank name surfaces name error`() = runTest {
        val vm = createViewModel()
        vm.setPetId("pet-1")
        vm.onDoseAmountChange("10")
        vm.onDoseUnitChange(DoseUnit.MG)
    ...
    @Test
    fun `save blank dose amount surfaces dose amount error`() = runTest {
        val vm = createViewModel()
        vm.setPetId("pet-1")
        vm.onNameChange("Amoxicillin")
        vm.onDoseUnitChange(DoseUnit.MG)
    ...
    @Test
    fun `save null dose unit surfaces dose unit error`() = runTest {
        val vm = createViewModel()
        vm.setPetId("pet-1")
        vm.onNameChange("Amoxicillin")
        vm.onDoseAmountChange("10")
    ...
    @Test
    fun `save null start date surfaces start date error`() = runTest {
        val vm = createViewModel()
        vm.setPetId("pet-1")
        vm.onNameChange("Amoxicillin")
        vm.onDoseAmountChange("10")
    ...
    @Test
    fun `save invalid phase surfaces phase error`() = runTest {
        val vm = createViewModel()
        vm.setPetId("pet-1")
        vm.onNameChange("Amoxicillin")
        vm.onDoseAmountChange("10")
        ...
        val result = vm.save()
        assertNull(result)
        assertNotNull(vm.state.value.phases[0].error)
    }

    @Test
    fun `save preflight failure surfaces form error`() = runTest {
        val vm = createViewModel()
        vm.setPetId("pet-1")
        vm.onNameChange("Amoxicillin")
        vm.onDoseAmountChange("10")
    ...
    @Test
    fun `field mutation clears own error`() = runTest {
        val vm = createViewModel()
        vm.setPetId("pet-1")
        vm.onNameChange("")
        vm.save()
    ...
    @Test
    fun `update phase recomputes night dose warning`() = runTest {
        val vm = createViewModel()
    ...
    @Test
    fun `affirm night dose clears warning`() = runTest {
        val vm = createViewModel()
        vm.updatePhase(0) { it.copy(doseTimes = listOf(LocalTime(3, 0))) }
        vm.affirmNightDose(0)
        assertEquals(false, vm.state.value.phases[0].nightDoseWarning)
    }

    @Test
    fun `add phase appends blank phase`() = runTest {
        val vm = createViewModel()
        vm.addPhase()
    ...
    @Test
    fun `remove phase removes at index`() = runTest {
        val vm = createViewModel()
        vm.addPhase()
        vm.removePhase(0)
