...
* **SqlDelight `ReminderLookup`** (M1.3): `SqlDelightReminderLookup` resolves dose-event ids to
  ...
  opens SQLDelight outside Koin. Green `SqlDelightReminderLookupContractTest` + Robolectric
  `DoseAlarmReceiverLookupTest`. ADR-0011 `fired_at` write-before-show deferred.
