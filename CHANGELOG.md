...
* **AppModule SQLDelight DI swap** (M1 step 3→4 bridge): `ToebeansDatabase` singleton with ADR-0010 `SqliteForeignKeysCallback`; `PetRepository` and `ScheduleRepository` bind to SQLDelight impls. `MedicationRepository` and `DoseEventRepository` remain fakes until follow-on M1 jobs.
